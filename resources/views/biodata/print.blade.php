...
<body>
    ...
    <table border="1"
        ...
        <tr>
            ...
            <td rowspan="11">
                @if ($data->foto != '-' || $data->foto != null)
                    <img src="{{ asset('uploads/' . $data->foto) }}" width="100%" height="100%" />
            </td>
            @endif
        ...
        <tr>
            ...
            <td width="45%" colspan="3"> <input type="checkbox" name="father" id="father"
                    {{ $data->status_ayah ? 'checked' : '' }}> 爸爸 father 年齡 :
                {{ $data->umur_ayah != null ? $data->umur_ayah : '0' }} 歲 <br> <input type="checkbox" name="ibu"
                    id="ibu" {{ $data->status_ibu ? 'checked' : '' }}> 媽媽 Mother 年齡 :
                {{ $data->umur_ibu != null ? $data->umur_ibu : '0' }} 歲</td>
        </tr>
        <tr>
        ...
        <tr>
            <td width="100%" colspan="5">是否有親友在台 ? <input type="checkbox" name="family_in_taiwan"
                    id="family_in_taiwan_yes" {{ $data->family_in_taiwan == '1' ? 'checked' : '' }}>Yes <input
                    type="checkbox" name="family_in_taiwan" id="family_in_taiwan_no"
                    {{ $data->family_in_taiwan != '1' ? 'checked' : '' }}>No <br>Do you have family / friend's working
                in Taiwan ?</td>
        </tr>
        <tr>
        ...
        @endif
        <tr>
            <td width="100%" colspan="5">備 註 NOTE</td>
        </tr>
                    ...
                    <td width="100%" colspan="5">
                        ...
                        @if ($q->Options()->count() > 0)
                            @foreach ($q->Options as $qo)
                                <input type="checkbox" id="biodata_answer_{{$qo->id}}" {{ $answer == $qo->id ? 'checked' : '' }}>{{$qo->option_text}}
                            @endforeach
                        @endif
                        <input type="checkbox" id="biodata_answer_0" {{ $answer == '0' ? 'checked' : '' }}> 其它說明: {{$customAnswer}}
                    </td>
                @endif
                @if ($q->type_question == '1')
                    <td width="100%" colspan="5">{{ $loop->index + 1 }}. {{ $q->question }} : {{$answer}}</td>
                @endif
